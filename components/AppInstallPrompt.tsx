...
export function AppInstallPrompt() {
  ...
  const primaryUrl = device === 'ios' ? IOS_APP_URL : device === 'android' ? ANDROID_APP_URL : null;
    ...
    <div className="mt-8 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className={device !== 'ios' ? 'flex-1' : 'hidden'}
        >
          <Button variant="outline" className="text-white border-white/20 font-semibold px-8 py-[14px] whitespace-nowrap h-[50px] w-full hover:bg-white/10 transition-all duration-200">
            <svg className="mr-2 h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
              <path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.12 2.65.72 3.4 1.8-3.12 1.87-2.38 5.98.48 7.13-.57 1.5-1.31 2.99-2.54 4.09l.01-.01zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z"/>
            </svg>
            Download for iOS
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className={device !== 'android' ? 'flex-1 relative' : 'hidden'}
        >
          <span className="absolute -top-2 -right-2 bg-yellow-500 text-black text-[10px] font-bold px-1.5 py-0.5 rounded-full uppercase z-10">Beta</span>
          <Button variant="outline" className="text-white border-white/20 font-semibold px-8 py-[14px] whitespace-nowrap h-[50px] w-full hover:bg-white/10 transition-all duration-200">
            <svg className="mr-2 h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
